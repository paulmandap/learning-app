...
| D14 | Not in the brief. §3.3 lists four things Gemini does; this is a fifth. | **A study assistant: one question, one answer, grounded in the student's own notes.** On a study screen it sees the card in front of you; anywhere else it sees the set. No conversation history, no scrollback. **20 questions per person per day**, claimed in the database before the model is called, never in the browser. The privacy copy names it (D13). | It spends the same free allowance card generation depends on, and generation is the product — an afternoon of chatting must not cost someone their cards. A counter in the browser would protect nothing: the quota is per Google project, so a phone and a laptop would each get a full allowance. History is dropped for the same reason — every follow-up re-sends the whole thread, so the third question costs several times the first, and each turn drifts further from the notes it is meant to be answering from. |
